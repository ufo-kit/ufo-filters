...
__kernel void lkflow( 
    ...
    __global float *guess_in,
    int guess_in_w,
    __global float *guess_out,
    int guess_out_w,
    int guess_out_h,
    int use_guess )
{
	// declare some shared memory
	__local float smem[2*FRAD + LOCAL_Y][2*FRAD + LOCAL_X] ;
	__local float smemIy[2*FRAD + LOCAL_Y][2*FRAD + LOCAL_X] ;
	__local float smemIx[2*FRAD + LOCAL_Y][2*FRAD + LOCAL_X] ;
	...
	smem[ tIdx.y ][ tIdx.x ] = read_imagef( I, nnSampler, Iidx+(float2)(-FRAD,-FRAD) ).x;
	smemIy[ tIdx.y ][ tIdx.x ] = read_imagef( Iy, nnSampler, Iidx+(float2)(-FRAD,-FRAD) ).x;
	smemIx[ tIdx.y ][ tIdx.x ] = read_imagef( Ix, nnSampler, Iidx+(float2)(-FRAD,-FRAD) ).x;

	// upper right
	if( tIdx.x < 2*FRAD ) { 
		smem[ tIdx.y ][ tIdx.x + LOCAL_X ] = read_imagef( I, nnSampler, Iidx+(float2)(LOCAL_X - FRAD,-FRAD) ).x;
		smemIy[ tIdx.y ][ tIdx.x + LOCAL_X ] = read_imagef( Iy, nnSampler, Iidx+(float2)(LOCAL_X - FRAD,-FRAD) ).x;
		smemIx[ tIdx.y ][ tIdx.x + LOCAL_X ] = read_imagef( Ix, nnSampler, Iidx+(float2)(LOCAL_X - FRAD,-FRAD) ).x;

	}
	// lower left
	if( tIdx.y < 2*FRAD ) {
		smem[ tIdx.y + LOCAL_Y ][ tIdx.x ] = read_imagef( I, nnSampler, Iidx+(float2)(-FRAD, LOCAL_Y-FRAD) ).x;
		smemIy[ tIdx.y + LOCAL_Y ][ tIdx.x ] = read_imagef( Iy, nnSampler, Iidx+(float2)(-FRAD, LOCAL_Y-FRAD) ).x;
		smemIx[ tIdx.y + LOCAL_Y ][ tIdx.x ] = read_imagef( Ix, nnSampler, Iidx+(float2)(-FRAD, LOCAL_Y-FRAD) ).x;

	}
	// lower right
	if( tIdx.x < 2*FRAD && tIdx.y < 2*FRAD ) {
		smem[ tIdx.y + LOCAL_Y ][ tIdx.x + LOCAL_X ] = read_imagef( I, nnSampler, Iidx+(float2)(LOCAL_X - FRAD, LOCAL_Y - FRAD) ).x;
		smemIy[ tIdx.y + LOCAL_Y ][ tIdx.x + LOCAL_X ] = read_imagef( Iy, nnSampler, Iidx+(float2)(LOCAL_X - FRAD, LOCAL_Y - FRAD) ).x;
		smemIx[ tIdx.y + LOCAL_Y ][ tIdx.x + LOCAL_X ] = read_imagef( Ix, nnSampler, Iidx+(float2)(LOCAL_X - FRAD, LOCAL_Y - FRAD) ).x;
    ...
    if( use_guess != 0 ) {
        ...
        int gin_x = iIidx.x/2;
        int gin_y = iIidx.y/2;
        float2 g_in = (float2)(guess_in[gin_y * guess_in_w + gin_x * 2 ], guess_in[gin_y * guess_in_w + gin_x * 2 + 1]);
	...
	float4 Gmat = read_imagef( G, nnSampler, iIidx );
    float det_G = Gmat.s0 * Gmat.s3 - Gmat.s1 * Gmat.s2 ;
    ...
    for( int k=0 ; k <8 ; k++ ) {
        ...
        for( int j=-FRAD ; j <= FRAD ; j++ ) {
            for( int i=-FRAD ; i<= FRAD ; i++ ) {
                ...
                //int Isample = read_imagef( I, nnSampler, Iidx+(float2)(i,j) ).x;
				float Isample = smem[tIdx.y + FRAD +j][tIdx.x + FRAD+ i];

                float Jsample = read_imagef( J_float,bilinSampler, Jidx+(float2)(i,j) ).x;
                float dIk = Isample - Jsample;
				float ix,iy;
				ix = smemIx[tIdx.y + FRAD +j][tIdx.x + FRAD+ i];
				iy = smemIy[tIdx.y + FRAD +j][tIdx.x + FRAD+ i];

                //ix = read_imagef( Ix, nnSampler, Iidx+(float2)(i,j)  ).x; 
				//iy = read_imagef( Iy, nnSampler, Iidx+(float2)(i,j)  ).x; 
    ...
    if( Iidx.x < guess_out_w && Iidx.y < guess_out_h ) {
        guess_out[ outCoords.y * guess_out_w + outCoords.x * 2] = v.x + g.x;
        guess_out[ outCoords.y * guess_out_w + outCoords.x * 2 + 1] = v.y + g.y;
    }
