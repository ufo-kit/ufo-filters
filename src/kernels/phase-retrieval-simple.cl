...
kernel void
tie_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
	output[idy * width + idx] = 1.0f / sin_arg;
}

kernel void
ctf_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
	output[idy * width + idx] = 1.0f / sin_value;
}

kernel void
ctfhalfsine_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
    output[idy * width + idx] = (sin_arg >= cutoff_frequency_low) ? 0.0f : 1.0f / sin_value;
}

kernel void
qp_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
    output[idy * width + idx] = (sin_arg > cutoff_frequency_low && fabs(sin_value) < thresholding_rate) ?
								0.0f : 1.0f / sin_value;
}

kernel void
qphalfsine_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
    output[idy * width + idx] = ((sin_arg > cutoff_frequency_low && fabs(sin_value) < thresholding_rate) || sin_arg >= cutoff_frequency_high) ?
								0.0f : 1.0f / sin_value;
}

kernel void
qp2_method(int normalize, float prefac, float regularize_rate, float thresholding_rate, float cutoff_frequency_low, float cutoff_frequency_high, global float *output)
{
    METHOD_ROUTINE;
    float cacl_filter_value = 0.5f * sign(sin_value) / (fabs(sin_value) + pow(10, -regularize_rate));
    output[idy * width + idx] = (sin_arg > cutoff_frequency_low && fabs(sin_value) < thresholding_rate) ?
								 sign(cacl_filter_value) / (2 * (thresholding_rate + pow(10, -regularize_rate))) : cacl_filter_value;
}
