...
static void ufo_filter_fft_initialize(UfoFilter *filter, UfoBuffer *params[])
{
    ...
    priv->fft_plan = clFFT_CreatePlan(
    ...
    guint dims[2] = {
        2 * priv->fft_size.x,
        priv->fft_dimensions == clFFT_1D ? priv->height : priv->fft_size.y
    };
...
static void ufo_filter_fft_process_gpu(UfoFilter *filter, UfoBuffer *params[])
{
    ...
    if (priv->fft_dimensions == clFFT_1D)
        clFFT_ExecuteInterleaved(command_queue,
                priv->fft_plan, (cl_int) priv->height, clFFT_Forward, 
                fft_buffer_mem, fft_buffer_mem,
                1, &wait_on_event, &event);
    else
        clFFT_ExecuteInterleaved(command_queue,
                priv->fft_plan, 1, clFFT_Forward, 
                fft_buffer_mem, fft_buffer_mem,
                1, &wait_on_event, &event);
    ...
    ufo_buffer_transfer_id(params[0], fft_buffer);
