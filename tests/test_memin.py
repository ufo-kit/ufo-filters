...
def get_opencl_buffer(width, height):
    ...
    ref_buf = pa.arange(queue, width * height, dtype=np.float32).reshape(height, width)
    ref = ref_buf.get()  # For checking later

    return (ref, ref_buf)


def run_test(input_dtype, use_complex=False, use_opencl=False):
    width = 8
    height = 4
    ...
    if use_opencl:
        try:
            in_numpy, buf = get_opencl_buffer(width, height)
        except ImportError:
            print("pyopencl not installed, skipping...")
            return
    else:
        in_numpy = np.arange(height * width).reshape(height, width).astype(input_dtype)
        if use_complex:
